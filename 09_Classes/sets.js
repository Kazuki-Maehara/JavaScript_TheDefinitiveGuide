...
* The member of this set are all values that are not members of some
...
* enmerable and does not have a meaningful size.
**/
...
* remove individual elements from the set, and then mplements concrete
...
class BitSet extends AbstractWritableSet {
  ...
  remove(x) {
    if (this._valid(x)) {
      let byte = Math.floor(x / 8);
      let bit = x % 8;
      // console.log(this.data[byte] , BitSet.bits[bit]);
      if (this._has(byte, bit)) {
        console.log(this.data[byte].toString(2), BitSet.masks[bit].toString(2));
        this.data[byte] &= BitSet.masks[bit];
        this.n--;
      }
    } else {
      throw new TypeError("Invalid set elemtns: " + x);
    }
  }
...
let myBitset = new BitSet(7);
console.log(myBitset);
myBitset.insert(0);
console.log(myBitset);
myBitset.insert(1);
console.log(myBitset);
